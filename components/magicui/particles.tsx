...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  const onMouseMove = () => {
    if (canvasRef.current && context.current) {
      ...
      if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
        ...
        // Reset to center if outside to prevent particles from disappearing
  ...
  const circleParams = (): Circle => {
    ...
    const alpha = 0.1; // Start with non-zero alpha
    const targetAlpha = parseFloat((Math.random() * 0.6 + 0.4).toFixed(1)); // Higher min alpha
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle, i) => {
      ...
      circle.alpha = Math.max(0.1, circle.targetAlpha * remapClosestEdge); // Ensure minimum alpha
